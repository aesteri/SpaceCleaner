...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int j = 0; j < height; j++)
  {
    ...
    DMA[3].src = &color;
    DMA[3].cnt = width | DMA_DESTINATION_INCREMENT | DMA_ON | DMA_SOURCE_FIXED;
  }
...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[3].cnt = WIDTH*HEIGHT | DMA_DESTINATION_INCREMENT | DMA_ON | DMA_SOURCE_INCREMENT;
}
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  {
    DMA[3].dst = &videoBuffer[OFFSET(row + j, col, WIDTH)]; 
    DMA[3].src = &image[OFFSET(row + j, col, WIDTH)];
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_FIXED;
  }
}
...
void fillScreenDMA(volatile u16 color) {
  ...
  DMA[3].dst = &videoBuffer[OFFSET(0 , 0, WIDTH)]; 
  DMA[3].src = &color;
  DMA[3].cnt = WIDTH*HEIGHT | DMA_DESTINATION_INCREMENT | DMA_ON | DMA_SOURCE_FIXED;
}
